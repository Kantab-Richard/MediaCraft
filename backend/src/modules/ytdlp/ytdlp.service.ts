...
import { BinPathType, DownloadKeyWord, DownloadOptions, Platform, ProgressType } from '../../types';
...
export class YtdlpService implements OnModuleInit {
  ...
  private async resolvePinterestUrl(url: string): Promise<string> {
    try {
      ...
      return await new Promise<string>((resolve) => {
        const request = https.get(
          ...
          (response) => {
            const redirectedUrl = (response as typeof response & { responseUrl?: string }).responseUrl;
            response.resume();
            resolve(redirectedUrl || url);
          },
        );
  ...
  private async ensureBinariesDirectory() {
    try {
      ...
      throw new Error(`Failed to create binaries directory: ${this.getErrorMessage(error)}`);
    }
  }
  ...
  private async setupBinaries() {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Failed to download binary files: ${this.getErrorMessage(error)}`);
      }
    }
  }
  ...
  private getOutputPath(
    ...
    if (typeof output === 'object') {
      const newObj: { outDir: string; filename: string } = {
        outDir: '',
        filename: '',
      };
      ...
      outputStr = path.join(newObj.outDir, newObj.filename ? newObj.filename : '%(title)s %(height)sp .%(ext)s');
    }
  ...
  async ytdlp(args: string[]): Promise<{ stdout: string; stderr: string }> {
    try {
      ...
      return await execFileAsync(this.ytdlpPath, processArgs);
    } catch (error) {
      throw new Error(`Failed to run yt-dlp command: ${this.getErrorMessage(error)}`);
    }
  }
  ...
  async getYtdlpVideoInfo(url: string): Promise<YtdlpVideoInfo> {
    const resolvedUrl = await this.resolvePinterestUrl(url);
    const command = [
      '--user-agent',
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
      '--socket-timeout',
  ...
  async getYoutubePlaylistInfo(url: string): Promise<PlaylistVideoInfo[]> {
    const command = ['--dump-json', '--no-download', '--flat-playlist', '--playlist-reverse', url];
    return this.ytdlp(command).then((result) => {
  ...
  async download<T extends DownloadKeyWord>(
    ...
    const normalizedUrl = platform === 'pinterest' ? await this.resolvePinterestUrl(url) : url;
    const parseUrl = parseAndValidateUrl(normalizedUrl, platform);
    ...
    childProcess.stderr.on('data', (data) => {
      const dataStr = Buffer.from(data).toString().trim();

      if (!dataStr) {
        return;
      }

      // yt-dlp prints many non-fatal warnings to stderr.
      if (dataStr.startsWith('WARNING:')) {
    ...
    childProcess.on('exit', (code) => {
      if (code !== 0 && !hasError) {
        subject.error(new Error(errorMessage || `Process exited with code ${code}`));
      } else if (!hasError) {
        subject.complete();
      }
    });
  ...
  async convertVideo(
    ...
    try {
      ...
      const { stdout } = await execAsync(`"${this.ffmpegPath}" -i "${inputPath}" 2>&1`).catch((err) => ({
        stdout: err.message,
      }));
      ...
      if (outputFormat === 'webm') {
        ...
        ffmpegArgs.push('-c:v', 'copy', '-c:a', 'copy', '-movflags', '+faststart');
      }
      ...
      process.stderr.on('data', (data) => {
        ...
        if (
          ...
          !dataStr.includes('Application provided invalid, non monotonically increasing dts')
        ) {
          hasError = true;
