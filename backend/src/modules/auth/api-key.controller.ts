import { Controller, Post, Body, Get, Param, Delete, Put, UnauthorizedException, Req } from '@nestjs/common';
...
export class ApiKeyController {
  ...
  async listAllApiKeys(@Req() req: Request) {
    ...
    const apiKeys = await Promise.all(users.map((user) => this.apiKeyService.listApiKeys(user.id)));
    return apiKeys.flat();
  }
