...
export class VideoDownload {
  private static downloads: Map<string, Subject<ProgressType | Error>> = new Map();
  private static clients: Map<string, Set<Response>> = new Map();
  private static progressTracker: Map<string, Record<string, number>> = new Map();
  private static progressUpdateCounter: Map<string, number> = new Map();
  private static downloadInfo: Map<string, { extension: string; filename: string }> = new Map();
  ...
  static createProgressSubject(downloadId: string, extension: string, filename: string): Subject<ProgressType | Error> {
      ...
      next: (progress) => {
        ...
        if (clients) {
          if (progress instanceof Error) {
            ...
            const updateCount = (this.progressUpdateCounter.get(downloadId) || 0) + 1;
  ...
  static getProgressSubject(downloadId: string): Subject<ProgressType | Error> | undefined {
  ...
  private static handleProgress(
    ...
    const updateCount = VideoDownload.progressUpdateCounter.get(downloadId) || 0;
