    ...
    origin: (origin: string, callback: (error: Error | null, success?: boolean) => void) => {
      ...
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          const devAllowed = ['localhost', '127.0.0.1', '192.168.', '10.', 'null'];
        ...
        allowedOrigins.push('medicraft.com', 'www.medicraft.com');

        if (allowedOrigins.some((domain) => originUrl.hostname.endsWith(domain))) {
          callback(null, true);
        } else {
    ...
    .addApiKey(
      {
        ...
        in: 'header',
        name: 'X-API-Key',
        description: 'API key for authentication. Can also be provided via Bearer token in Authorization header.',
      },
      'X-API-Key',
    )
    .addBearerAuth({
      ...
      description: 'API key as Bearer token. Can also be provided via X-API-Key header.',
    })
    .build();
